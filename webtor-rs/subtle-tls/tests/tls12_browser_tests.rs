...
mod tls12_crypto_tests {
    ...
    async fn test_verify_data_computation() {
        ...
        let client_finished = prf::compute_verify_data(
            &master_secret,
            true,
            &handshake_hash
        ).await.unwrap();
        
        let server_finished = prf::compute_verify_data(
            &master_secret,
            false,
            &handshake_hash
        ).await.unwrap();
...
mod tls12_handshake_tests {
    ...
    use subtle_tls::handshake_1_2::{
        Handshake12State, CipherSuiteParams,
    ...
    async fn test_all_supported_cipher_suites() {
        ...
        for (suite, expected_key_len, expected_iv_len, is_aead) in suites {
            ...
            assert_eq!(params.key_len, expected_key_len, "Key length mismatch for 0x{:04x}", suite);
            assert_eq!(params.iv_len, expected_iv_len, "IV length mismatch for 0x{:04x}", suite);
            assert_eq!(params.is_aead, is_aead, "AEAD flag mismatch for 0x{:04x}", suite);
        }
    }
    ...
    async fn test_cipher_suite_key_block_length() {
        ...
        let gcm_params = CipherSuiteParams::for_suite(TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256).unwrap();
        assert_eq!(gcm_params.key_block_len(), 40);
        
        // AES-256-GCM: 0 + 32 + 32 + 4 + 4 = 72
        let gcm256_params = CipherSuiteParams::for_suite(TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384).unwrap();
        assert_eq!(gcm256_params.key_block_len(), 72);
        
        // AES-128-CBC-SHA256: 32 + 32 + 16 + 16 + 0 + 0 = 96
        let cbc_params = CipherSuiteParams::for_suite(TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256).unwrap();
...
mod tls12_record_tests {
    ...
    async fn test_unencrypted_record_roundtrip() {
        ...
        layer.write_record(&mut record, CONTENT_TYPE_HANDSHAKE, data).await.unwrap();
    ...
    async fn test_gcm_encrypted_record_roundtrip() {
        let mut layer = RecordLayer12::new();
        layer.set_cipher_suite(TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256).unwrap();
        
        let key = vec![0x42u8; 16];
        let implicit_iv = vec![0x01u8; 4];
        let mac_key = vec![];  // GCM doesn't use MAC key
        
        layer.set_write_cipher(&key, &implicit_iv, &mac_key).await.unwrap();
        layer.set_read_cipher(&key, &implicit_iv, &mac_key).await.unwrap();
    ...
    async fn test_multiple_records_sequence_numbers() {
        ...
        for i in 0..outputs.len() {
            for j in (i + 1)..outputs.len() {
                assert_ne!(outputs[i].0, outputs[j].0, "Records {} and {} should differ", i, j);
            }
        }
...
mod tls_version_negotiation_tests {
    ...
    async fn test_tls_version_enum() {
        ...
        assert_eq!(TlsVersion::Prefer13, TlsVersion::Prefer13);
